...
"""
AWS Resource Notifier Lambda Function

This Lambda function processes CloudTrail events for AWS resource creation
...
def get_resource_info(event_detail):
    ...
    if event_source == 'ec2.amazonaws.com' and event_name == 'RunInstances':
        ...
        # Extract instance ID and type from response elements
        for item in event_detail.get('responseElements', {}).get('instancesSet', {}).get('items', []):
        ...
        if instance_id:
            try:
                # First check if tags are in the event
                tag_specifications = event_detail.get('requestParameters', {}).get('tagSpecificationSet', {}).get('items', [])
                ...
                # If not all tags found in event, try to get from EC2 API
                if not tags or not instance_name:
                    ...
                    for tag in response.get('Tags', []):
                        if tag.get('Key') == 'Name' and not instance_name:
                            instance_name = tag.get('Value')
                        tags[tag.get('Key')] = tag.get('Value')
            except Exception as e:
        ...
        resource_info = {
            ...
            'additionalInfo': {
                ...
                'availabilityZone': event_detail.get('availability-zone', 'Unknown'),
    ...
    # S3 Bucket Creation
        ...
        try:
            # Give S3 a moment to create the bucket before fetching tags
    ...
    # RDS Instance Creation
        ...
        db_engine = request_params.get('engine')
        
        # Try to extract tags from request parameters
        tags = {}
        tags_list = request_params.get('tags', [])
        for tag in tags_list:
            tags[tag.get('key')] = tag.get('value')
        
        # If no tags in request, try to get from RDS API
        if not tags:
            try:
    ...
    # Lambda Function Creation
        ...
        # If no tags in request, try to get from Lambda API
        if not tags:
            try:
    ...
    # Security Group Creation
        ...
        group_id = event_detail.get('responseElements', {}).get('groupId')
        tags = {}
        
        # Try to get security group tags - they might be in a TagSpecification
        try:
            # First check if tags are in the event
            tag_specifications = request_params.get('tagSpecificationSet', {}).get('items', [])
            ...
            # If not found in event or incomplete, try EC2 API
            if not tags:
                response = ec2_client.describe_tags(
                    Filters=[
                        {
        ...
        resource_info = {
            ...
            'resourceId': group_id,
            'additionalInfo': {
                'vpcId': request_params.get('vpcId', 'Default VPC'),
                'description': request_params.get('groupDescription', 'No description'),
                'tags': tags if tags else 'No tags found'
            }
        }
    ...
    # VPC Creation
        ...
        cidr_block = response_elements.get('vpc', {}).get('cidrBlock')
        tags = {}
    ...
    # Elastic Load Balancer Creation (ALB/NLB)
        ...
        lb_scheme = request_params.get('scheme', 'internet-facing')
        
        # Extract ARN for LB
        lb_arn = None
        if response_elements and 'loadBalancers' in response_elements:
            for lb in response_elements.get('loadBalancers', []):
                lb_arn = lb.get('loadBalancerArn')
                if not lb_name or lb_name == 'Unknown':
                    lb_name = lb.get('loadBalancerName', 'Unknown')
                break
        
        # Subnet information
        subnets = request_params.get('subnets', [])
        subnet_info = ", ".join(subnets) if subnets else "Unknown"
        
        # Try to get tags from request parameters
        tags = {}
        tags_list = request_params.get('tags', [])
        for tag in tags_list:
            tags[tag.get('key')] = tag.get('value')
        
        # If not found in event or incomplete, try ELB API
        if not tags and lb_arn:
            try:
        ...
        resource_info = {
            'resourceType': f'Elastic Load Balancer ({lb_type.upper()})' if lb_type != 'Unknown' else 'Elastic Load Balancer',
            'resourceName': lb_name,
            'resourceId': lb_arn if lb_arn else lb_name,
            'additionalInfo': {
                'type': lb_type,
                'scheme': lb_scheme,
                'subnets': subnet_info,
...
def get_service_icon(resource_type):
    ...
    # For ELB resource types that may include the load balancer type in parentheses
    if resource_type.startswith('Elastic Load Balancer'):
...
def create_teams_message(event, resource_info):
    ...
    message = {
        ...
        "themeColor": "0078D4", 
        "summary": f"{resource_info['resourceType']} Created",
        "sections": [
            {
                "activityTitle": "",
                "activitySubtitle": "",
...
<div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px;">
    <div style="background: linear-gradient(135deg, #0078D4 0%, #83c6ff 100%); border-radius: 12px; padding: 20px; color: white; margin-bottom: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
        <div style="display: flex; align-items: center; justify-content: space-between;">
            <div>
                <h2 style="margin: 0; font-size: 22px; font-weight: 600;">✅ NEW RESOURCE CREATED</h2>
        ...
        <div style="margin-top: 20px; display: flex; justify-content: space-between; align-items: flex-end;">
            ...
            <div style="text-align: right;">
                <p style="margin: 0; font-size: 14px; opacity: 0.9; font-weight: 500;">CREATED BY</p>
    ...
    <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
        <h3 style="margin: 0 0 15px 0; color: #0078D4; font-size: 16px; border-bottom: 1px solid #eee; padding-bottom: 8px;">📊 RESOURCE DETAILS</h3>
    ...
    if resource_info['additionalInfo']:
        ...
        <h3 style="margin: 15px 0 15px 0; color: #0078D4; font-size: 16px; border-bottom: 1px solid #eee; padding-bottom: 8px;">📙 ADDITIONAL INFORMATION</h3>
        <div style="display: flex; flex-wrap: wrap;">
        """
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # For Lambda function creation events, we often get multiple identical events
        # We'll only process the event if it contains responseElements with functionArn
        event_source = event.get('detail', {}).get('eventSource', '')
        event_name = event.get('detail', {}).get('eventName', '')
        
        if event_source == 'lambda.amazonaws.com' and event_name == 'CreateFunction20150331':
            response_elements = event.get('detail', {}).get('responseElements', {})
            # Only process Lambda creation events that have a complete response
            if not response_elements or 'functionArn' not in response_elements:
                logger.info("Skipping incomplete Lambda creation event")
                return {
                    'statusCode': 200,
                    'body': 'Skipped intermediate Lambda creation event'
                }
        ...
        if send_successful:
            return {
                'statusCode': 200,
                'body': f"Notification sent for {resource_info['resourceType']} creation: {resource_info['resourceName']}"
            }
        else:
            return {
                'statusCode': 500,
