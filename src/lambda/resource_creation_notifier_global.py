...
"""
AWS Resource Creation Notifier Global (us-east-1) Lambda Function
...
def get_resource_info(event_detail):
    ...
    # IAM User Creation
    if event_source == 'iam.amazonaws.com' and event_name == 'CreateUser':
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Skip processing if resource_info is None (e.g., intermediate Lambda creation events)
        if resource_info is None:
            logger.info("Skipping event processing as it's not the final creation event")
            return {
                'statusCode': 200,
                'body': 'Event skipped - not the final creation event'
            }
